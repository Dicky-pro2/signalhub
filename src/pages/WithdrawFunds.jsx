...
export default function WithdrawFunds() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // Try withdrawal_requests first, fall back to withdrawals
    ...
    try {
      ...
      const { error: withdrawError } = await supabase
        ...
        .insert({
          ...
          account_number:
            selectedMethod === "bank" ? bankDetails.accountNumber : cryptoAddress,
          account_name:
            selectedMethod === "bank"
      ...
      const { error: txError } = await supabase.from("transactions").insert({
        ...
        description: `Withdrawal via ${
          selectedMethod === "bank" ? `Bank Transfer (${bankDetails.bankName})` : selectedCrypto
        }`,
      });
      ...
      setSuccess(
        `Withdrawal request of $${amount.toFixed(2)} submitted successfully! Processing in 1-3 business days.`
      );
      setWithdrawAmount("");
      ...
      setTimeout(() => navigate("/provider/earnings"), 3000);
    } catch (err) {
        ...
        <form onSubmit={handleWithdraw} className="space-y-6">
          ...
          {selectedMethod === "bank" && (
            <div>
              ...
              >
                ⚠️ Bank transfers take 1-3 business days. Please ensure your account
                details are correct.
              </div>
            </div>
          )}
            ...
            <div>
              ...
              >
                <p className="font-semibold mb-1">
                  📌 Network:{" "}
                  {cryptoOptions.find((c) => c.value === selectedCrypto)?.network}
                </p>
                <p>
                  ⚠️ Send only {selectedCrypto} to this address. Wrong network = permanent
                  loss of funds.
                </p>
              </div>
            </div>
          ...
          {[
            {
              ...
              a: "Bank transfers: 1-3 business days. Cryptocurrency: Instant to 1 hour.",
            },
            {
              q: "What are the fees?",
              a: "Bank transfers: Free. Cryptocurrency: 1% fee. PayPal: 2% fee (coming soon).",
            },
            {
              q: "Why is my balance pending?",
              a: "Earnings from signal sales are held for 7 days to protect buyers and ensure signal quality.",
            },
            {
              q: "Can I cancel a withdrawal?",
              a: "Withdrawals can be cancelled if still in pending state. Contact support immediately.",
            },
          ].map((faq, i) => (
            <div key={i}>
