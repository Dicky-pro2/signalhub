...
export default function MarketplacePage() {
  ...
  const handleConfirmPurchase = async () => {
    ...
    try {
      ...
      // Refresh signals or update the purchased signal
      fetchSignals();
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      {topProviders.length > 0 && (
        <div className={`rounded-xl p-4 ${darkMode ? 'bg-gradient-to-r from-orange-600/20 to-red-600/20 border border-orange-500/30' : 'bg-gradient-to-r from-orange-50 to-red-50 border border-orange-200'}`}>
          <h3 className={`font-semibold mb-3 flex items-center gap-2 ${darkMode ? 'text-white' : 'text-gray-800'}`}>
            <span>🏆</span> Top Rated Providers This Week
          </h3>
          <div className="flex gap-4 overflow-x-auto pb-2">
            {topProviders.map(provider => (
              <div key={provider.id} className={`rounded-lg px-4 py-2 min-w-[160px] ${darkMode ? 'bg-gray-800/80' : 'bg-white shadow-sm'}`}>
                <p className={`font-medium ${darkMode ? 'text-white' : 'text-gray-800'}`}>
                  {provider.full_name}
                </p>
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-yellow-400 text-sm">★ {provider.rating.toFixed(1)}</span>
                  <span className="text-green-400 text-xs">{provider.win_rate}% win rate</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className={`max-w-md w-full rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-white'} p-6`}>
            ...
            <div className={`p-4 rounded-lg mb-4 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
              ...
              <div className="flex justify-between mb-2">
                ...
                <span className="text-orange-500 font-bold">${selectedSignal.price}</span>
              </div>
              <div className="flex justify-between pt-2 border-t border-gray-600">
                <span className={darkMode ? 'text-gray-400' : 'text-gray-500'}>Wallet Balance:</span>
                <span className="text-green-500 font-bold">$47.50</span>
              </div>
            </div>

            <div className={`text-sm mb-4 p-3 rounded-lg ${darkMode ? 'bg-yellow-500/10 text-yellow-400' : 'bg-yellow-50 text-yellow-600'}`}>
            ...
            <div className="flex gap-3">
              ...
              >
                {buying ? 'Processing...' : `Confirm - $${selectedSignal.price}`}
              </button>
              <button
...
function SignalCard({ signal, darkMode, onBuy }) {
  ...
  // Mock purchase for demo - in real app, this would come from API
  const handleLocalBuy = () => {
    onBuy();
    // For demo purposes only - remove in production
    if (window.confirm('Demo: Reveal full analysis?')) {
      setFullAnalysis(signal.analysis_full || 'This is the detailed analysis including technical indicators, support/resistance levels, and risk management tips for this trade setup.');
      setPurchased(true);
    }
  };
      ...
      <div className={`rounded-xl border-2 border-green-500/50 overflow-hidden ${darkMode ? 'bg-gray-800' : 'bg-white shadow-lg'}`}>
        <div className="p-5">
          ...
          <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-4 mb-4">
            <p className="text-green-400 text-sm font-semibold mb-2">📋 Full Analysis Revealed:</p>
