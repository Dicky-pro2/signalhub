      ...
      const { data, error } = await supabase
        ...
        .select(`
          *,
          ...
          signals(
            id, asset, title, signal_type, entry_price,
            target_price, stop_loss, description, status,
            timeframe, result
          )
        `)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className={card}>
          ...
          <p className={`text-2xl font-bold ${text}`}>${totalSpent.toFixed(2)}</p>
        </div>
        <div className={card}>
        ...
        <div className={card}>
          ...
          <p className={`text-2xl font-bold ${text}`}>
            {purchases.filter(p => p.status === 'cancelled' || p.status === 'expired').length}
          </p>
        </div>
      </div>
