...
export default function AdminDashboard() {
  ...
  const [pendingProvidersList, setPendingProvidersList] = useState([
    { id: 1, name: 'John Smith', email: 'john@example.com', experience: '5 years forex trading', submittedAt: '2024-01-14', winRate: '76%', documents: true },
    { id: 2, name: 'Emma Wilson', email: 'emma@example.com', experience: '3 years crypto trading', submittedAt: '2024-01-14', winRate: '82%', documents: true },
    { id: 3, name: 'Michael Chen', email: 'michael@example.com', experience: '8 years stocks', submittedAt: '2024-01-13', winRate: '71%', documents: false },
    { id: 4, name: 'Sarah Johnson', email: 'sarah@example.com', experience: '4 years forex and crypto', submittedAt: '2024-01-12', winRate: '88%', documents: true },
  ]);

  const [flaggedSignals, setFlaggedSignals] = useState([
    { id: 1, pair: 'DOGE/USD', provider: 'MemeMaster', price: 19.99, reports: 3, reason: 'Price too high', status: 'pending', createdAt: '2024-01-15' },
    { id: 2, pair: 'SHIB/USD', provider: 'ShillKing', price: 49.99, reports: 5, reason: 'Suspicious analysis', status: 'pending', createdAt: '2024-01-14' },
    { id: 3, pair: 'PEPE/USD', provider: 'FrogTrader', price: 29.99, reports: 2, reason: 'Misleading entry', status: 'reviewing', createdAt: '2024-01-13' },
  ]);
  ...
  const [recentTransactions, setRecentTransactions] = useState([
    ...
    { id: 4, user: 'CryptoKing', type: 'payout', amount: 342.50, status: 'processing', date: '2024-01-14' },
    { id: 5, user: 'AIInvestor', type: 'withdrawal', amount: 200.00, status: 'completed', date: '2024-01-13' },
  ]);

  const [allUsers, setAllUsers] = useState([
    { id: 1, name: 'John Doe', email: 'john@example.com', role: 'customer', status: 'active', joined: '2024-01-01', spent: 47.50 },
    { id: 2, name: 'Jane Smith', email: 'jane@example.com', role: 'provider', status: 'active', joined: '2024-01-02', earned: 1247.89 },
    { id: 3, name: 'Bob Johnson', email: 'bob@example.com', role: 'customer', status: 'suspended', joined: '2024-01-03', spent: 12.99 },
    { id: 4, name: 'Alice Brown', email: 'alice@example.com', role: 'provider', status: 'pending', joined: '2024-01-04', earned: 0 },
  ]);

  // Actions
  const approveProvider = (id) => {
    setPendingProvidersList(pendingProvidersList.filter(p => p.id !== id));
    setStats({ ...stats, pendingProviders: stats.pendingProviders - 1, totalProviders: stats.totalProviders + 1 });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <p className={`text-xs text-green-500 mt-1`}>+{stats.monthlyGrowth}% growth</p>
        </motion.div>
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Platform Fees</span>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6">
          ...
          <div className={`rounded-xl p-6 ${darkMode ? 'bg-gray-800' : 'bg-white shadow-lg'}`}>
            ...
            <div className="mt-4 text-center">
              <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                Total this week: <span className="text-orange-500 font-bold">$15,820</span>
              </p>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <div className={`rounded-xl p-4 ${darkMode ? 'bg-gray-800' : 'bg-white shadow-lg'} text-center`}>
              <Icon icon={Icons.UserPlus} size={32} color="#f97316" className="mx-auto mb-2" />
              <h3 className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-800'}`}>Pending Approvals</h3>
            ...
            <div className={`rounded-xl p-4 ${darkMode ? 'bg-gray-800' : 'bg-white shadow-lg'} text-center`}>
              ...
              <h3 className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-800'}`}>Flagged Signals</h3>
            ...
            <div className={`rounded-xl p-4 ${darkMode ? 'bg-gray-800' : 'bg-white shadow-lg'} text-center`}>
              ...
              <h3 className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-800'}`}>Pending Payouts</h3>
        ...
        <div className={`rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-white shadow-lg'} p-6`}>
          ...
          {pendingProvidersList.length === 0 ? (
            <div className="text-center py-8">
              <Icon icon={Icons.Verified} size={48} color="#22c55e" className="mx-auto mb-3" />
              <p className={darkMode ? 'text-gray-400' : 'text-gray-500'}>No pending applications</p>
            </div>
          ) : (
      ...
      {activeTab === 'signals' && (
        <div className={`rounded-xl ${darkMode ? 'bg-gray-800' : 'bg-white shadow-lg'} p-6`}>
          ...
          {flaggedSignals.length === 0 ? (
            <div className="text-center py-8">
              <Icon icon={Icons.Success} size={48} color="#22c55e" className="mx-auto mb-3" />
              <p className={darkMode ? 'text-gray-400' : 'text-gray-500'}>No flagged signals</p>
            </div>
          ) : (
            <div className="space-y-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded-full ${
                          signal.status === 'pending' ? 'bg-yellow-500/20 text-yellow-500' : 'bg-orange-500/20 text-orange-500'
                        }`}>
                          {signal.status}
                        </span>
                      </div>
                  ...
                  <tr key={user.id} className="border-b border-gray-700/50">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-1 rounded-full ${
                        user.role === 'provider' ? 'bg-orange-500/20 text-orange-500' : 'bg-blue-500/20 text-blue-500'
                      }`}>
                        {user.role}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-1 rounded-full ${
                        ...
                        user.status === 'suspended' ? 'bg-red-500/20 text-red-500' : 'bg-yellow-500/20 text-yellow-500'
                      }`}>
                        {user.status}
                      </span>
                    </td>
                    ...
                    <td className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                      {user.role === 'provider' ? `$${user.earned} earned` : `$${user.spent} spent`}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <button
              ...
              <tbody>
                {recentTransactions.map((tx) => (
                  <tr key={tx.id} className="border-b border-gray-700/50">
                    <td className={`py-3 font-medium ${darkMode ? 'text-white' : 'text-gray-800'}`}>{tx.user}</td>
                    <td className="py-3 capitalize">{tx.type}</td>
                    <td className={`py-3 font-semibold ${
                      tx.type === 'deposit' ? 'text-green-500' : 
                      tx.type === 'withdrawal' ? 'text-red-500' : 'text-orange-500'
                    }`}>
                    ...
                    <td className="py-3">
                      <span className={`text-xs px-2 py-1 rounded-full ${
                        ...
                        tx.status === 'pending' ? 'bg-yellow-500/20 text-yellow-500' : 'bg-blue-500/20 text-blue-500'
                      }`}>
                        {tx.status}
                      </span>
                    </td>
                    <td className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>{tx.date}</td>
                    <td className="py-3">
